...
vector<int> Merge(vector<vector<int>> data) {
    ...
    while(true)
    {
        ...
        {   
            ...
            while(approx_pos > 0 and data[smallest][approx_pos] > data[second_smallest][pos[second_smallest]]){
                cdf_error += 1;
                approx_pos -= 1;
            }
        while((approx_pos) < data[smallest].size() && (approx_pos <= 0 || data[smallest][approx_pos] <= data[second_smallest][pos[second_smallest]])){
            cdf_error += 1;
            approx_pos += 1;
        }
